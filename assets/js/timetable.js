...
function showClosedGuard() {
	if (globalContain) {
		tiva_timetables.push({
			"name": "c̸̤̗͖̑̈́̾̓̌̔͠C̸̞̩͍͉̈́͛̿̔͝ͅL̶̨͉̞̣͔̣͍̠̥̤͛̕Ǫ̵̫̗̯̫͎̎̌͊̈́̚͠S̶̙̺͇̬̟̙̟̠̀̈Ḛ̵̠̠͉̱͓̯̾̈̈́̋̓̚D̴̛̗͇̿̎̌̐̂͒͌͂̚ ̸̡̡̟̝̳͎͑͋̾̀̔̎̿̎G̷̛̛̛̱͍̺̯͙͈̜͚̯̞͆̍̈U̸̡͇̖̬̹͍̘̚Ą̸̩̆̃̽͊̃͋̉͐̍̌R̵͔̩͚̼̬̐̽́̿͛̌̈́͝͠͝D̴̞͚͔̯̮̭̃̀͠",
			"image": "closedguard.jpg",
			"day": "sunday",
			"start_time": "12:00",
			"end_time": "14:00",
			"color": "6",
		...
		var todayDate = new Date();
		var date_start = (typeof globalContain.attr('data-start') != "undefined") ? globalContain.attr('data-start') : 'sunday';
		if (date_start == 'sunday') {
			var tiva_current_week = new Date(todayDate.setDate(tiva_current_date.getDate() - todayDate.getDay()));
		} else {
			var today_date = (todayDate.getDay() == 0) ? 7 : todayDate.getDay();
		...
		createTimetable(globalContain, 'current', tiva_current_week, tiva_current_month, tiva_current_year);
	}
}